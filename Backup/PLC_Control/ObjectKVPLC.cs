...
namespace PLC_Control
{
    ...
    public class Object_Trigger
    {
        ...
        {
            this_data = value;
            //Console.WriteLine(DateTime.Now.TimeOfDay + " checkTrigger: " + value);     
    ...
    class ObjectPLC_KV
    {
        ...
        public bool checkConnect(bool check, bool show)
        {
            ...
            {
                //顯示目前狀態
                if (axDBCommManager.Active == false)
        ...
        public bool doConnect()
        {
            ...
            {
                if (ex.Message.Contains("Failed to open the port.") == true) 
                    Console.WriteLine("PLC連線失敗");
                    //MessageBox.Show("PLC連線失敗");
                else
        ...
        public bool checkTrigger(bool check, bool show)
        {
            if (_TriggerSource == TriggerSource.ALWAYS)
            {
                ...
                //顯示目前狀態
                if (_ThreadTrigger == null)
                    Console.WriteLine("未開啟接收觸發");
                    //MessageBox.Show("未開啟接收觸發");
                else
                    Console.WriteLine("已開啟接收觸發");
                    //MessageBox.Show("已開啟接收觸發");
                return false;
            }
            ...
            {
                //顯示目前狀態
                if (axDBTriggerManager.Active == false)
                    Console.WriteLine("未開啟接收觸發");
                    //MessageBox.Show("未開啟接收觸發");
                else
                    Console.WriteLine("已開啟接收觸發");
